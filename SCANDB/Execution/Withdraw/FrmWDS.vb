...
Public Class FrmWDS
    ...
    Private Sub Cmd_datediff_Click(sender As Object, e As EventArgs) Handles cmd_datediff.Click
        ...
        sql = $"SELECT EXEWDS.*,EXECHECK.CHKTOTALEXERE,EXECHECK.CHKDATESEND,DATEDIFF(DAY,EXEDATEWDS,GETDATE()) AS SEND_LATE,EXECHECK.CHKDETAIL1 FROM EXEWDS LEFT JOIN EXECHECK ON EXEWDS.EXECUSACC = EXECHECK.CHKACC WHERE EXEDATEWDS IS NOT NULL AND EXECHECK.CHKDATESEND IS NULL AND EXECHECK.CHKDETAIL1 LIKE '%คชจ.%' ORDER BY EXEDATEWDS;"
        cmd = New SqlCommand(sql, cn)
        DA = New SqlDataAdapter(cmd)
        DS = New DataSet
        DA.Fill(DS, "datelate")
